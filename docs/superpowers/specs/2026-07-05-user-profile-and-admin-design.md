...
- 배경: 소셜 로그인으로 `profiles`에 사용자가 쌓이기 시작했으나 (1) 실명·전화·주소 등 프로필 정보를 수집하지 않고 (2) 운영자가 사용자를 조회할 화면이 없다. 투자자 요청상 사용자 정보 수집이 비즈니스 요구사항.

## 1. 목표와 범위

두 서브 기능을 하나의 설계로 묶되 구현 순서는 A → B.

- **서브 A — 프로필 수집**: OAuth 가입 직후, 동의 화면(`/consent`)에서 실명·전화·성별·주소를 **필수 입력**받아 저장.
...
alter table profiles
  ...
  add column if not exists address    text;   -- 기본 주소
```

- **판정 헬퍼**: `hasCompleteProfile(profile)` — `full_name`, `phone`, `gender`, `address`가 모두 non-empty면 완료. `lib/auth/profileFields.ts`(신규)에 정의.
...
- `ConsentForm`에 **프로필 입력 섹션** 추가: 이름(text), 전화(text, 010 형식), 성별(라디오 male/female/unspecified), 주소(text).
- **[동의하고 시작] 활성 조건**: 기존 `requiredDone`(필수 동의 3개) **AND** 프로필 4개 입력 완료.
- 전화번호 형식 검증: `01[016789]-?\d{3,4}-?\d{4}` 수준의 클라이언트 검증. 실패 시 안내.
- 제출 → `POST /api/auth/consent` 확장: body에 `{ marketing, profile: { full_name, phone, gender, address } }`. 서버가 동의 타임스탬프 + 프로필 필드를 함께 `profiles` update.
...
- `app/(admin)/admin/users/page.tsx` — client, TanStack Table. 컬럼: 이메일 · 이름(full_name) · 성별 · 전화 · 주소 · 가입일 · 필수동의(✓/✗, `hasRequiredConsent` 재사용) · 마케팅(✓/—). 주소는 길어질 수 있어 셀에서 말줄임(ellipsis) 처리하고 `title` 속성으로 전체 노출.
...
이름·전화·주소·성별은 개인정보다. `/consent`의 **"개인정보 수집·이용" 동의 항목이 이 수집 항목을 근거로 커버**해야 한다(개인정보 보호법 제15조: 수집 목적·항목·보유 기간·거부권 고지).

- 동의 문구에 수집 항목(이름·전화·성별·주소)과 목적(전문가 상담 연결·본인 식별)을 명시해야 함.
