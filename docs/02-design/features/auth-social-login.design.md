...
## 7. 메인 진입 UX (홈 화면 인증 배치) — 확정 2026-06-14

로그인 입구는 **헤더 1곳**, 전문가 모집은 **별개·저순위 CTA**. SOS 핵심 루프(입력→추천→전화)와 시각적으로 경쟁시키지 않는다 (D6). "사용자/전문가 로그인"은 분리된 버튼이 아니라 **동일 소셜 로그인 1개**이며, 차이는 로그인 후 `role` 라우팅뿐이다 (§2).

### 7.1 헤더 우상단 — 로그인/계정 입구

```text
비로그인:  [SOS 골고루]   전문가 둘러보기 >   [ 로그인 ]
로그인됨:  [SOS 골고루]   전문가 둘러보기 >   [🟢 김○○ ▾]
                                              └ 내 기록 · 찜한 전문가
                                                 전문가 등록 신청 · 로그아웃
```

- **비로그인**: `로그인` 텍스트 칩(저채도·작게). 클릭 → 로그인 바텀시트.
- **로그인됨**: 같은 자리를 아바타 + 드롭다운으로 교체.
- `전문가 둘러보기` 링크는 기존 유지(인증 무관, 비로그인 열람 가능).

### 7.2 로그인 = 바텀시트 (공용 컴포넌트)

- 하단에서 올라오는 시트: **Google · Kakao** 2버튼 + 약관·개인정보 안내 1줄.
- 진입점 2개가 같은 시트 공유: ① 헤더 `로그인` 칩, ② 보호 액션(찜·상담요청·내 기록)에서 lazy 호출 (§3).
- `returnTo` 보존 → 로그인 후 원위치·원액션 복귀, 익명 데이터 병합 (D7).
- 전용 `/login` 페이지는 MVP 제외(화면 전환 마찰 회피).

### 7.3 전문가 등록 = 모집 CTA (로그인과 별개·저순위)

- **비로그인 (주 진입점)**: `/experts`(전문가 둘러보기) 하단 전용 푸터에 `전문가이신가요? 골고루 입점 신청 →` (저순위 톤). 디렉터리에서 동료 프로필을 보는 맥락이 입점 전환에 가장 강함.
- **홈 푸터**: 112·119 안내 **전용** — 전문가 CTA 미노출(소비자 SOS 루프 집중, D6).
- **로그인 후**: 계정 드롭다운의 `전문가 등록 신청`.
- 클릭 → (미로그인 시 로그인 시트 먼저) → 등록 신청 폼(직역·자격·이름·전화) → `expert_applications`(pending) → 관리자 승인 후 `role=expert` 승격 (§4, D4).

### 7.4 관리자

- 메인에 **노출 안 함**. 기존 `/admin/login`(이메일+비밀번호) 유지 (D5).

---

## 8. 구현 시 체크리스트 (Post-설계)
...
- [ ] 로그인 바텀시트 컴포넌트 (Google·Kakao, 헤더 칩 + 보호 액션 공용, returnTo 보존 — §7.2)
- [ ] `/my` 전문가 대시보드 (미니홈피 데이터 재사용 — `/expert/[id]` 컴포넌트 공유)
- [ ] 어드민 전문가 신청 승인 화면 + `expert.approve/reject` AuditAction
- [ ] 메인 헤더: 비로그인 `로그인` 칩 / 로그인 시 아바타+드롭다운(내 기록·찜·전문가 등록·로그아웃) — §7.1
- [ ] 푸터 전문가 입점 CTA + 계정메뉴 `전문가 등록 신청` → 신청 폼 (§7.3)
